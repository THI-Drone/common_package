...
struct JsonKeyDefinition {
  ...
  static std::string data_type_to_string(const data_type_t data_type) {
    ...
    case number_unsigned:
      return "uint";
    default:
      throw std::runtime_error("JsonKeyDefinition::data_type_to_string: "
   ...
   * specified, all values will be accepted. Every data type that is not of any
   * `number*` data type will always return true.
   *
   * @param json_iter The JSON iterator to check.
   * @return True if the JSON iterator is within the bounds or the data type is
   * not supported, false otherwise.
   */
   ...
   *
   * This constructor initializes a JsonKeyDefinition object with the specified
   * parameters. Use this constructor if you want to allow multiple data types.
   *
   ...
   *
   * This constructor initializes a JsonKeyDefinition object with the specified
   * parameters. Use this constructor if you just want to allow one data type.
   *
   * @param required A boolean indicating whether the key is required or not.
...
public:
   ...
   *
   * The keys in the map are of type std::string, representing the names of the
   * command keys. The values in the map are of type JsonKeyDefinition,
  ...
  get_waypoint_command_definition() {
    const std::map<const std::string, const JsonKeyDefinition> definition{
        ...
        {"cruise_height_cm",
         {true, number_unsigned, MIN_CRUISE_HEIGHT_CM, MAX_FLIGHT_HEIGHT_CM}},
