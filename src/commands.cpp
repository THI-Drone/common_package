...
nlohmann::json CommandDefinitions::parse_check_json(
  ...
  {
    bool unknown_key_found = false; // If true, unknown keys were found and an
  ...
  for (const auto &[key, json_definition] : definition) {
    ...
    if (search == json_obj.end()) {
      if (json_definition.required) // Check that key exists if required
        throw std::runtime_error(
            "CommandDefinitions::parse_check_json: Missing required key: " +
            key);
    ...
    if (!json_definition.check_bounds(search)) {
      std::string error_msg = "CommandDefinitions::parse_check_json: [Key: '" +
                              key + "'] Value '" + search->dump() +
                              "' is out of bounds: ";

      if (json_definition.min_val.has_value() &&
          json_definition.max_val
              .has_value()) // Message when min and max values are defined
        error_msg += "[" + std::to_string(json_definition.min_val.value()) +
                   ...
                   .has_value()) // Message when only the min value is defined
        error_msg += "> " + std::to_string(json_definition.min_val.value());
      else if (json_definition.max_val
                   .has_value()) // Message when only the max value is defined
    ...
    for (const auto &data_type :
         json_definition.data_types) // Loop through all the allowed data types
    {
      switch (data_type) {
      case null:
        if (search->is_null())
      ...
      case number_unsigned:
        ...
        break;
      default:
        throw std::runtime_error("CommandDefinitions::parse_check_json: "
    ...
    if (!type_check) // Type check failed
    {
      ...
      for (const auto &data_type :
           json_definition.data_types) // Loop through all allowed data types
      ...
      std::string error_msg =
          "CommandDefinitions::parse_check_json: [Key: '" + key + "'] Value '" +
          search->dump() +
